...
$request = static function (string $path, ?array $post = null, bool $ajax = false, bool $secure = true) use ($client, $runtime): array {
    ...
    echo 'HTTP ' . $status . ' ' . parse_url($path, PHP_URL_PATH) . ' -> ' . parse_url($headers['location'] ?? '', PHP_URL_PATH) . PHP_EOL;
...
if (200 !== $login['status'] || false !== ($data['hasErrors'] ?? null)) {
...
$invalidCode = substr($code, 0, 5) . (string) (((int) substr($code, -1) + 1) % 10);
$response = $request($challengeUrl, ['one_time_code' => ['code' => $invalidCode, '_token' => $csrf, 'submit' => '']]);
$check(200 === $response['status'] && (int) $repository->factor($employeeId)['last_counter'] < $counter,
    'an invalid TOTP cannot advance the counter or verify the session');
...
// Use the native route collection for every supported modern action; cURL executes no JavaScript.
$router = new Symfony\Component\Routing\Generator\UrlGenerator($kernel->getContainer()->get('router')->getRouteCollection(), new Symfony\Component\Routing\RequestContext());
$modernAction = static function (string $route, array $parameters = []) use ($router, $employeeId): string {
    return '/admin-dev/index.php' . $router->generate($route, $parameters) . '?token=' . Tools::getAdminToken($employeeId);
};
$mutations = [];
foreach (['install', 'uninstall', 'reset', 'enable', 'disable', 'enable_mobile', 'disable_mobile', 'upgrade'] as $action) {
    $mutations['Symfony ' . $action] = [$modernAction('admin_module_manage_action', [
        'action' => $action, 'module_name' => 'mp2fa_missing_request_fixture',
    ]), []];
}
foreach (['install', 'uninstall', 'configure', 'enable', 'disable', 'reset', 'upgrade'] as $action) {
    $mutations['Symfony bulk ' . $action] = [$modernAction('admin_module_manage_action_bulk', ['action' => $action]),
        ['module_name' => ['mp2fa_missing_request_fixture']]];
}
$mutations['Symfony import'] = [$modernAction('admin_module_import'), []];
$mutations['Symfony update all'] = [$modernAction('admin_module_manage_update_all'), []];
$legacyModules = '/admin-dev/index.php?controller=AdminModules&token='
    . Tools::getAdminToken('AdminModules' . (int) Tab::getIdFromClassName('AdminModules') . $employeeId);
foreach (['install', 'uninstall', 'reset', 'delete', 'enable', 'disable', 'upgrade', 'import'] as $action) {
    $mutations['legacy GET ' . $action] = [$legacyModules . '&' . $action . '=mp2fa_missing_request_fixture', null];
    $mutations['legacy POST ' . $action] = [$legacyModules, ['action' => $action, 'module_name' => 'mp2fa_missing_request_fixture']];
    $mutations['legacy bulk ' . $action] = [$legacyModules, ['submitBulk' . $action . 'module' => 1]];
}
$mutations['legacy case-insensitive controller'] = [str_replace('AdminModules&', 'adminmodules&', $legacyModules) . '&install=mp2fa_missing_request_fixture', null];
$mutations['legacy benign action cannot hide install'] = [$legacyModules . '&action=view&install=mp2fa_missing_request_fixture', null];
$moduleBefore = Db::getInstance()->executeS('SELECT id_module, name, active, version FROM ' . _DB_PREFIX_ . 'module ORDER BY id_module');
echo 'Waiting for the configured step-up window to expire.' . PHP_EOL;
sleep(61);
foreach ($mutations as $label => $mutation) {
    $response = $request($mutation[0], $mutation[1]);
    $check(302 === $response['status']
        && false !== strpos($response['headers']['location'] ?? '', '/mpadmin2fa/challenge')
        && false !== strpos($response['headers']['location'] ?? '', 'step_up=1'),
        $label . ' requires fresh MFA without JavaScript');
}
$check($moduleBefore === Db::getInstance()->executeS(
    'SELECT id_module, name, active, version FROM ' . _DB_PREFIX_ . 'module ORDER BY id_module', true, false
), 'denied module actions leave installed module records unchanged');
...
$deniedAuditBefore = (int) Db::getInstance()->getValue(
    ...
    . ' AND metadata_json LIKE "%native_permission_missing%"'
);
...
$employeesUrl = '/admin-dev/index.php/modules/mpadmin2fa/enrollment?token=' . Tools::getAdminToken($employeeId);
...
try {
    ...
    if ('ok' !== $access->updateLgcAccess($superAdminProfileId, $enrollmentTabId, 'delete', true, false)) {
        throw new RuntimeException('Could not restore the native delete permission.');
    }
}
$check(302 === $response['status'] && null !== $repository->factor($resetEmployeeId),
...
try {
    ...
    $delegatedApprovalActionQuery = [];
    parse_str((string) parse_url($delegatedApprovalActionUrl, PHP_URL_QUERY), $delegatedApprovalActionQuery);
    $check(200 === $response['status'] && '' !== $delegatedApprovalActionUrl && '' !== $delegatedApprovalActionToken
        && !isset($delegatedApprovalActionQuery['mp2fa_csrf_token'])
        && !in_array($delegatedApprovalActionToken, $delegatedApprovalActionQuery, true),
