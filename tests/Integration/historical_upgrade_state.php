...
if (in_array($action, ['repeat', 'repair'], true)) {
    require_once $root . '/modules/mpadmin2fa/upgrade/upgrade-3.0.0rc1.php';
    $assert(upgrade_module_3_0_0rc1($module), 'the RC upgrade entry point can run twice');
}
$assert('3.0.0rc1' === $module->version && $module->version === $version, 'native upgrade advanced the installed version to the RC');
