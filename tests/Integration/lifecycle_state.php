...
// This harness changes state only in an explicitly disposable shop.
...
$verifyCleanup = static function () use ($assertCount, $prefix, $moduleSql): void {
    ...
    foreach (['access', 'module_access'] as $table) {
        $assertCount('orphan ' . $table, 'SELECT COUNT(*) FROM ' . $prefix . $table . ' a'
            ...
            // Some stock PS 1.7 fixtures contain a legacy zero-role access row.
...
switch ($action) {
    ...
    case 'verify-reconciliation':
        $module = Module::getInstanceByName('mpadmin2fa');
        $before = $database->getRow('SELECT * FROM ' . $prefix . 'mp2fa_keyring WHERE active = 1');
        if (!$module->upgradeSecurityRemediation() || !$module->upgradeSecurityRemediation()) {
            throw new RuntimeException('Repeated reconciliation failed.');
        }
        $tab = new Tab((int) Tab::getIdFromClassName('AdminMpAdmin2faSecurityActivity'));
        $execute('DELETE FROM ' . $prefix . 'tab_lang WHERE id_tab = ' . (int) $tab->id);
        $execute('DELETE FROM ' . $prefix . 'tab WHERE id_tab = ' . (int) $tab->id);
        Tab::resetStaticCache();
        if (!$module->unregisterHook('actionDispatcherBefore')) {
            throw new RuntimeException('Could not create a partially missing state.');
        }
        $execute('DELETE a FROM ' . $prefix . 'access a INNER JOIN ' . $prefix . 'authorization_role r'
            . ' ON r.id_authorization_role = a.id_authorization_role'
            . ' WHERE r.slug = "ROLE_MOD_TAB_ADMINMPADMIN2FAAUTHENTICATOR_READ"');
        $execute('DROP TABLE ' . $prefix . 'mp2fa_rate_limit');
        Configuration::deleteByName(Mpadmin2fa\Security\Policy::CONFIG_AUDIT_DAYS);
        if (!$module->upgradeSecurityRemediation()) {
            throw new RuntimeException('Partially missing state could not be repaired.');
        }
        $after = $database->getRow('SELECT * FROM ' . $prefix . 'mp2fa_keyring WHERE active = 1', false);
        if ($before !== $after) {
            throw new RuntimeException('Reconciliation changed the encryption key.');
        }
        break;

    case 'prepare-partial-uninstall':
        $module = Module::getInstanceByName('mpadmin2fa');
        $tabId = (int) Tab::getIdFromClassName('AdminMpAdmin2faSecurityActivity');
        $execute('DELETE FROM ' . $prefix . 'tab_lang WHERE id_tab = ' . $tabId);
        $execute('DELETE FROM ' . $prefix . 'tab WHERE id_tab = ' . $tabId);
        $execute('DROP TABLE ' . $prefix . 'mp2fa_rate_limit');
        Configuration::deleteByName(Mpadmin2fa\Security\Policy::CONFIG_AUDIT_DAYS);
        if (!$module->unregisterHook('actionDispatcherBefore')) {
            throw new RuntimeException('Could not prepare partial-state uninstall.');
        }
        break;
