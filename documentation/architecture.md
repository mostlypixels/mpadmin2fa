# Architecture for PrestaShop 1.7.8
...
This branch supports 1.7.8 only and PHP 7.1 through 7.4.
The code does not use PHP syntax that is newer than PHP 7.1.
...
This branch uses Symfony 3.4.
The security adapter uses `InteractiveLoginEvent` and `GetResponseEvent`.
The repository uses the Doctrine DBAL 2 result API.

PrestaShop 1.7.8 uses Symfony 3.4.
The subscriber uses the interactive-login event and the kernel request event.
It reads the employee from the security token storage.
The controller uses annotation access rules.
The service file gives explicit arguments and command tags to the older container.
The repositories use the Doctrine DBAL 2 result API.

The key service includes a protected-key rewrapper.
This adapter supports the Defuse key format that PrestaShop 1.7.8 supplies.
