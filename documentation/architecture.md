...
| --- | --- |
| **PHP syntax** | Syntax compatible with PHP 7.1 |
| **Symfony** | 3.4 |
| **Login events** | `InteractiveLoginEvent` and `GetResponseEvent` |
| **Database API** | Doctrine DBAL 2 result API |
| **Controller access rules** | Annotation access rules |
| **Service setup** | Explicit arguments and command tags |

The key service supports the Defuse key format supplied by PrestaShop 1.7.8.
...
The module runs on PHP 7.1 through 7.4. The build excludes documentation, tests, tools, and the normal Composer vendor directory.
